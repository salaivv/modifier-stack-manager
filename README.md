...
2. Create a `.env` file in the repo's root directory and add the full path to Blender executable in the format `BLENDER=/path/to/blender` to this file.
3. Create a new blend file named `testing.blend` in the repo's root directory.
4. Copy the code in `testing.py` to the Text Editor in `testing.blend`.
5. As you make changes to the code, press the _Run Script_ ▶️ button in the Text Editor to build and reinstall the addon.
